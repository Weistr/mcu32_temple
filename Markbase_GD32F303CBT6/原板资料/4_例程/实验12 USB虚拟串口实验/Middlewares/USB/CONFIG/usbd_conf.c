...
void HAL_PCD_MspInit(PCD_HandleTypeDef* pcdHandle)
{
		if(pcdHandle->Instance==USB)
		{
			...
			rcu_usb_clock_config(RCU_CKUSB_CKPLL_DIV2_5); //USB时钟为48M,系统时钟2.5分频
			rcu_periph_clock_enable(RCU_USBD);            //使能USBD时钟
			
			//PA11,12复用推挽输出
			gpio_init(GPIOA, GPIO_MODE_AF_PP, GPIO_OSPEED_50MHZ, GPIO_PIN_11 | GPIO_PIN_12);
			//USB中断NVIC配置
			nvic_irq_enable(USBD_LP_CAN0_RX0_IRQn, 0, 3); //抢占优先级0，子优先级3
		}
}
...
void HAL_PCD_MspDeInit(PCD_HandleTypeDef* pcdHandle)
{
		if(pcdHandle->Instance==USB)
		{
			rcu_periph_clock_disable(RCU_USBD);            //失能USBD时钟
			 
			nvic_irq_disable(USBD_LP_CAN0_RX0_IRQn);       //失能中断
		}
}
